...
func randomCommentID() string                   { return RandomCommentID() }
func randomReplyID() string                     { return RandomReplyID() }
func stampWithFocus(c Comment, f Focus) Comment { return StampWithFocus(c, f) }
...
func diffHunksForFile(path, oldPath, status, baseRef, repoRoot string, ignoreWhitespace bool, vc vcs.VCS) ([]vcs.DiffHunk, error) {
	return diffHunksForFileCtx(context.Background(), path, oldPath, status, baseRef, repoRoot, ignoreWhitespace, vc)
}

func diffHunksForFileCtx(ctx context.Context, path, oldPath, status, baseRef, repoRoot string, ignoreWhitespace bool, v vcs.VCS) ([]vcs.DiffHunk, error) {
	if status == "renamed" && oldPath != "" {
		return vcs.FileDiffUnifiedCtx(ctx, path, oldPath, baseRef, repoRoot, ignoreWhitespace)
	}
	if v != nil {
		return v.FileDiffUnifiedCtx(ctx, path, baseRef, repoRoot, ignoreWhitespace)
	}
...
type Session struct {
	VCS            vcs.VCS // nil for non-VCS sessions (e.g. files mode without a repo)
	Files          []*FileEntry
...
func populateEagerFile(fe *FileEntry, fc vcs.FileChange, baseRef, root string, vc vcs.VCS) bool {
	...
	if fc.Status != "deleted" {
		if fc.Status == "added" || fc.Status == "untracked" {
			...
			populateEagerFileDiff(fe, fc, baseRef, root, vc)
		}
	}
...
func populateEagerFileDiff(fe *FileEntry, fc vcs.FileChange, baseRef, root string, vc vcs.VCS) {
	hunks, err := diffHunksForFile(fc.Path, fc.OldPath, fc.Status, baseRef, root, false, vc)
	if err != nil {
		fmt.Fprintf(os.Stderr, "Warning: diff failed for %s: %v\n", fc.Path, err)
	} else {
		fe.DiffHunks = hunks
...
// It delegates to NewGitSession with a vcs.GitVCS backend.
// Retained for test compatibility; production code uses NewGitSession directly.
func NewSessionFromGit(ignorePatterns []string) (*Session, error) {
	return NewGitSession(&vcs.GitVCS{}, ignorePatterns)
}
...
func NewGitSession(v vcs.VCS, ignorePatterns []string) (*Session, error) {
	return newGitSession(v, ignorePatterns, true)
}

// NewGitSessionLenient is like NewGitSession but tolerates an empty working tree.
// Used when --pr/--range focus will rebuild the file list via SetFocus.
func NewGitSessionLenient(v vcs.VCS, ignorePatterns []string) (*Session, error) {
	return newGitSession(v, ignorePatterns, false)
}

// newGitSession is the implementation behind NewGitSession. When
...
func newGitSession(v vcs.VCS, ignorePatterns []string, requireChanges bool) (*Session, error) {
	root, err := v.RepoRoot()
	if err != nil {
	...
	if errors.Is(err, ErrNoChangedFiles) && !requireChanges {
		...
		resolvedBase = v.DefaultBranch()
		if branch != resolvedBase {
			baseRef, _ = v.MergeBase(v.DefaultBaseRef())
		}
		err = nil
	}
	if err != nil {
	...
	genRules, _ := vcs.ParseGeneratedRules(root)
	...
	for i, fc := range changes {
		...
		fe := &FileEntry{
			...
			Generated: vcs.IsGenerated(fc.Path, s.generatedRules),
		}
...
// canonicalAbsPath resolves p to an absolute path and evaluates symlinks.
// On macOS, /var/folders/... and /private/var/folders/... refer to the same
// location; normalizing here keeps filepath.Rel against git's repo root stable.
func canonicalAbsPath(p string) (string, error) {
	...
	if err != nil {
		return abs, nil //nolint:nilerr // best-effort: path may not exist yet
	}
	return resolved, nil
}

// canonicalRepoRoot evaluates symlinks on a VCS repo root so it matches
// canonicalAbsPath output from expandAndDedupPaths.
...
func expandAndDedupPaths(paths []string, ignorePatterns []string) ([]string, error) {
	...
	for _, p := range paths {
		absPath, err := canonicalAbsPath(p)
		if err != nil {
...
// resolveGitContext returns vcs.VCS repo state for file-mode sessions.
func resolveGitContext() (root, branch, baseRef, baseBranchName string, vc vcs.VCS) {
	vc = vcs.DetectVCS("")
	if vc == nil {
		return "", "", "", "", nil
	}
	...
	return root, branch, baseRef, baseBranchName, vc
}
...
func NewSessionFromFiles(paths []string, ignorePatterns []string) (*Session, error) {
	...
	root, branch, baseRef, baseBranchName, vc := resolveGitContext()
	if root == "" {
		root = filepath.Dir(expandedPaths[0])
	}
	root = canonicalRepoRoot(root)
	...
	for _, absPath := range expandedPaths {
		...
		fe := &FileEntry{
			...
			Comments:  []Comment{},
			Generated: vcs.IsGenerated(relPath, s.generatedRules),
		}

		if vc != nil {
			hunks, diffErr := vc.FileDiffUnified(relPath, baseRef, root, false)
			if diffErr != nil {
				fmt.Fprintf(os.Stderr, "Warning: diff failed for %s: %v\n", relPath, diffErr)
			} else {
				fe.DiffHunks = hunks
...
func fileIgnored(full, root string, ignorePatterns []string) bool {
	...
	for _, pat := range ignorePatterns {
		if config.MatchPattern(pat, relPath) {
			return true
		}
	}
	return false
...
func dirIgnored(full, root string, ignorePatterns []string) bool {
	...
	for _, pat := range ignorePatterns {
		...
		if config.MatchPattern(pat, dirPath) {
			return true
		}
	}
	return false
...
func filterBinary(changes []vcs.FileChange) []vcs.FileChange {
...
func (s *Session) AddComment(filePath string, startLine, endLine int, side, body, quote, author, userID string) (Comment, bool) {
	...
	if side == "old" && s.BaseRef != "" {
		...
		if s.VCS != nil {
			...
			baseContent = vcs.FileContentAtRef(filePath, s.BaseRef, s.RepoRoot)
		}
...
func (s *Session) EnsureFileEntry(path string) bool {
	...
	status := "modified"
	if v != nil {
		status = v.FileStatusInRepo(path, baseRef, repoRoot)
	}
	...
	fe := &FileEntry{
		...
		FileHash:  fileHash(data),
		Comments:  []Comment{},
		Generated: vcs.IsGenerated(path, s.generatedRules),
	}

	// Generate diff hunks
	if status == "added" || status == "untracked" {
		fe.DiffHunks = vcs.FileDiffUnifiedNewFile(fe.Content)
	} else if status != "deleted" && v != nil {
		if hunks, err := v.FileDiffUnified(path, baseRef, repoRoot, false); err == nil {
			fe.DiffHunks = hunks
		}
	}
...
func (s *Session) ChangeBaseBranch(branch string) error { //nolint:gocyclo // inherent complexity: rollback, recompute diffs, preserve comments
	...
	mb, err := v.MergeBase(branch)
	if err != nil {
		mb, err = v.MergeBase("origin/" + branch)
		if err != nil {
	...
	oldOverride := v.GetDefaultBranchOverride()

	// Update the override so ChangedFiles() uses the new base
	v.SetDefaultBranchOverride(branch)
	...
	if currentBranch != branch {
		changes, err = v.ChangedFilesFromBaseInDir(mb, repoRoot)
	} else {
		changes, err = v.ChangedFilesOnDefaultInDir(repoRoot)
	}
	if err != nil {
		// Rollback all state
		v.SetDefaultBranchOverride(oldOverride)
		s.mu.Lock()
	...
	for _, fc := range changes {
		...
		if fc.Status != "added" && fc.Status != "untracked" {
			if hunks, diffErr := v.FileDiffUnified(fc.Path, mb, repoRoot, false); diffErr == nil {
				fe.DiffHunks = hunks
			}
		} else {
			fe.DiffHunks = vcs.FileDiffUnifiedNewFile(fe.Content)
...
func (s *Session) loadSnapshotsFromSidecar(identity string) bool {
	...
	sf, err := loadSnapshotsFromDisk(sidecarPath)
	if err != nil {
...
func whitespaceIgnoredHunks(cached []vcs.DiffHunk, status, oldPath string, ignoreWhitespace bool, path, baseRef, repoRoot string, vc vcs.VCS) []vcs.DiffHunk {
	...
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	hunks, err := diffHunksForFileCtx(ctx, path, oldPath, status, baseRef, repoRoot, true, vc)
	if err != nil {
...
func (s *Session) GetFileDiffSnapshot(path string, ignoreWhitespace bool) (map[string]any, bool) {
	...
	if prevContent != "" {
		entries := diff.ComputeLineDiff(prevContent, currContent)
